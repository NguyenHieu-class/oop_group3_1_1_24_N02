...
void FolderView::loadDirectory(const string path, bool effectRibbon)
{
    ...
    // Chuyển đổi path sang wstring
    std::wstring wpath = stringToWString(path);
    unsigned long attribute = GetFileAttributesW(wpath.c_str());
    if (attribute & FILE_ATTRIBUTE_DIRECTORY)
    {
        ...
        findFileHandle = FindFirstFileW((wpath + L'*').c_str(), &findFileData);
        ...
        if (findFileHandle != INVALID_HANDLE_VALUE)
        {
            do
            {
                if ((findFileData.dwFileAttributes & FILE_ATTRIBUTE_HIDDEN) == 0 && findFileData.cFileName[0] != L'.')
                {
                    ...
                    if (isDirectory)
                    {
                        filePath = path + std::string(findFileData.cFileName, findFileData.cFileName + wcslen(findFileData.cFileName)) + '\\';
                    }
                    else
                    {
                        filePath = path + std::string(findFileData.cFileName, findFileData.cFileName + wcslen(findFileData.cFileName));
                    ...
                    files.emplace_back(File(filePath,
                        std::string(findFileData.cFileName, findFileData.cFileName + wcslen(findFileData.cFileName)),
...
void FolderView::loadDrives()
{
    ...
    unsigned long int stringsSize = GetLogicalDriveStringsW(bufSize, drivesBuf);
    ...
    while (pos < stringsSize)
    {
        ...
        {
            drives.emplace_back(Drive(std::string(tempBuf, tempBuf + wcslen(tempBuf))));
            tempPos = 0;
        }
        else
        {
