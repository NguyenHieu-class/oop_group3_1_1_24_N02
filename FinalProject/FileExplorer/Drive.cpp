...
void Drive::refreshStatistics()
{
    ...
    // Chuyển đổi rootPath sang wstring
    std::wstring wRootPath = stringToWString(rootPath);
    if (GetDiskFreeSpaceExW(wRootPath.c_str(), &freeDiskBytes, &totalDiskBytes, nullptr))
    {
...
{
    wchar_t volumeNameBuf[300], fileSystemNameBuf[100];
    ...
    {
        label = std::string(volumeNameBuf, volumeNameBuf + wcslen(volumeNameBuf));
        fileSystemName = std::string(fileSystemNameBuf, fileSystemNameBuf + wcslen(fileSystemNameBuf));
        driveType = GetDriveTypeW(wRootPath.c_str());
        return true;
    }
    driveType = GetDriveTypeW(wRootPath.c_str());
